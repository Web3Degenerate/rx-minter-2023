...
// import { CreateImagingOrder } from ".";
...
//useRef() remove lag solution: https://dev.to/kevinkh89/how-to-solve-input-delay-lagging-in-react-j2o
  ...
  //******** medication_Ref IS NOW ORDER/INDICATION CALLED 'medication_order_indication_Ref' */
  let medication_order_indication_Ref = inputMedication.current.value;
                      console.log('Medication (Now ORDER/INDICATION) is:', medication_order_indication_Ref)


  //******** medication_Ref IS NOW ORDER/INDICATION CALLED 'medication_order_indication_Ref' */
  let dea_quantity_Dx_Code_Ref = inputQuantity.current.value;
  // quantity_Ref will be set to 1
  let quantity_Ref = 0;
...
// Set initial "Next Fill Date" plus 1 #FixNextFill
  ...
  let nextFillDate = date_prescribed_Ref;


//*********************************************************************** START OF MERGE FN TEST ************************************************/
if (confirm(`Proceed minting an Imaging Order for "${medication_order_indication_Ref}" for patient ${name} with the SIG of ${description_Ref} on
 ${formatDateTwoDigitYear(date_prescribed_Ref)} with the qualifying Dx Code(s) of ${dea_quantity_Dx_Code_Ref}? 
 (You will select the facility to receive this order after it is minted to the blockchain).  
  Do you wish to proceed?`) == true){                       
try {


//https://www.youtube.com/live/bBjZUe56biU?feature=share&t=1135
// 1 - wallet_address (string)      (Pt wallet_address) ===> address (logged in doctor)
// 2 - patient name (string)
// 3 - description / sig (string)
// 4 - medication (string) (medication_Ref) ======> medication_order_indication_Ref 
// 5 - dob (string)
...
// 12 - nextFillDate (uint256)  
  ...
  const sendRx = await contract.call("mintRx", [address]); 
  ...
  alert(`The imaging order for patient ${name} has been minted and sent to your address ${addyShortner(address)} with a transaction ID of ${sendRx.receipt.transactionHash}`); 
...
<hr></hr>
                      ...
                      <Link style={{color:"#74A27F"}} to={`/fax-imaging-order/${newTokenId}`} >
...
<hr></hr>
                  ...
                  <div className="row">
                          ...
                          <div className="col-md-9">
                              ...
                              <textarea name="pt_physical_address" className="form-control" value={pt_physical_address} rows="3" disabled/>
...
<hr></hr>
{/* MEDICATION CHANGED TO FREE TEXT ON THU 7/7/23. MED = ORDER */}
                  ...
                  <div className="row">
                          ...
                          <div className="col-md-9">
                              ...
                              {/* <input type="text" name="description" className="form-control" value={form.description} onChange={(e) => handleFormFieldChange('description', e)} />   */}
                              {/* <input type="text" name="description" className="form-control" value={form.description} onChange={(e) => handleFormFieldChange(e)} /> */}
                              <textarea name="medication" placeholder="Indication goes here" className="form-control" ref={inputMedication} rows="2" onKeyDown={handleKeyDown} />
                          </div>
                  </div>
                {/* <div className="row">         
...
<hr></hr>
{/* QUANTITY PRESCRIBED ************  QUANTITY ===> DX CODES */}
                  <div className="row">
                          <div className="col-md-3">Dx Code(s):</div>
                          <div className="col-md-9">
                              {/* <input type="text" name="name" className="form-control" value={name} onChange={(e) => handleChange(e)} /> */}             
                              {/* <input type="text" name="name" className="form-control" value={form.name} onChange={(e) => handleChange(e)} />   */}
                              ...
                              <input type="text" name="quantity" placeholder="Qualifying Dx Code(s) go here" className="form-control" ref={inputQuantity} /> 
                          </div>
                  </div>


<hr></hr>
{/* HIDE DAYS SUPPLY (set to 1) PRESCRIBED */}
                  {/* <div className="row">
                          <div className="col-md-6">Days Supply:</div>
                          <div className="col-md-6">
                              <input type="number" name="max" className="form-control" ref={inputMax} /> 
                          </div>
                  </div>                */}
...
export default CreateImagingOrder